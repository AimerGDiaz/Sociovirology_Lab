...
qPCRs_fil_avg <- ddply(qPCRs_raw, .(Primer, Construct,Sample_Group), summarize, Avg_Ct = mean(CQ) )


qPCRs_PP2A <- qPCRs_fil_avg[qPCRs_fil_avg$Primer =="PP2A",]
qPCRs_Target <- qPCRs_fil_avg[qPCRs_fil_avg$Primer !="PP2A",]

qPCRs_DCt <- merge(x = qPCRs_PP2A , y = qPCRs_Target, by =c("Construct","Sample_Group"))
...
#TuMV_RE <- ggplot(qPCRs_DCt, aes(x = Construct, y = RelExpr, fill = Construct , group =  Construct)) +
TuMV_RE <- ggplot(qPCRs_DCt[!grepl("FEM|ERV|5",qPCRs_DCt$Sample_Group),], aes(x = Construct, y = RelExpr, fill = Construct , group =  Construct)) +
#TuMV_RE <- ggplot(qPCRs_DCt[grepl("_TA",qPCRs_DCt$Sample_Group),], aes(x = Construct, y = RelExpr, fill = Construct , group =  Construct)) +
     geom_bar(position = position_dodge(width = 0.9), stat = 'summary', fun.data = mean_se, alpha = 0.6) +
geom_errorbar(position = position_dodge(width = 0.9),
                stat = 'summary', fun.data = mean_se, width = 0.4) + facet_wrap(~ Primer.y , scales = "free")  + 
   ylab("Relative expression to PP2A")   +  xlab("Plant agroinfiltrated") +
...
#ggplot(qPCRs_DDCt[!grepl("TA", qPCRs_DDCt$Sample_Group ),], 
       ggplot(qPCRs_DDCt[grepl("_4", qPCRs_DDCt$Sample_Group ),], 
#ggplot(qPCRs_DDCt[grepl("_TA", qPCRs_DDCt$Sample_Group ),], 
          aes(x = Construct, y = log2DDCt, fill = Construct , group =  Construct)) +
     geom_bar(position = position_dodge(width = 0.9), stat = 'summary', fun.data = mean_se, alpha = 0.6) +
geom_errorbar(position = position_dodge(width = 0.9),
                stat = 'summary', fun.data = mean_se, width = 0.4) + facet_wrap(~ Primer.y.x , scales = "free")  + 
   ylab("Relative expression to PP2A")   +  xlab("Construct") +
     geom_text_repel( color ="red",
             aes(label = Sample_Group,  hjust = -0.05, vjust = -0.05)) +
   geom_point(position = position_dodge(width = 0.9), alpha = 0.5, size = 0.8, color = "grey20" ) + theme_classic(base_size = 14) +
   theme(axis.text.x = element_text(angle = 90,size = 14)) 
